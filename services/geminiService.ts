...
export const askSkillLift = async (history: ChatMessage[], newMessage: string): Promise<string> => {
  try {
    ...
    const dynamicSystemInstruction = `${baseSystemInstruction} You have access to a database of ${opportunities.length} scholarships and programs. When users ask about these topics, reference the specific information you have.`;
...
export const getRealtimeInspiration = async (query: string): Promise<InspirationalPerson | null> => {
    ...
    const schema = {
        ...
        properties: {
            ...
            category: { type: Type.STRING, enum: ['Historical Pioneers', 'Modern Innovators', 'Nobel Laureates', 'Rising Stars'] },
    ...
    try {
        ...
        const jsonText = response.text.trim();
        const result = JSON.parse(jsonText) as InspirationalPerson;
        return result;

    } catch (error) {
        console.error("Error getting realtime inspiration:", error);
        logError(error, { function: 'getRealtimeInspiration' });
        return null;
    }
};
